...
export function createKamera({ nivaer, startId, onNiva }) {
  ...
  async function gaTill(id) {
    ...
    // att måla lagret INNAN kameran börjar röra sig (#374). Ett kallt/nybyggt
    // lager (klassbyn: ~25 hus-SVG:er + emoji-avatarer) kostar annars en
    // style/paint-spik som äter upp övergångens första frames.
    // OBS – mätt och förkastat: att i stället för-visa lagret i SLUTLÄGET
    // scale(1) ("rastrera skarpt en gång, GPU-skala sedan") hjälper INTE –
    // modern Chromium om-rastrerar will-change-lager vid skalbyte ändå, och
    // den gamla scale(1)-rastern hann dessutom SYNAS suddigt i zoomstartens
    // första frames. Avslöja därför alltid i startläget, aldrig med ett
    // skal-hopp. I gömd flik tas hela resan som direkthopp (gaTill ovan).

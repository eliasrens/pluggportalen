...
export async function pageElevVarld(startNiva) {
  ...
  function renderPalett() {
    const aktivtRum = roomCtl ? roomCtl.current() : 0;
    const nuvarande = roomCtl ? (roomCtl.wallPaletteId() || husPaletteId) : husPaletteId;
    ...
    renderPalettePicker(palettRad, nuvarande, (id) => {
      ...
      if (roomCtl) roomCtl.setWallPalette(id);
      // Rum 0:s palett är även husets exteriör → uppdatera utsidan.
      if (!roomCtl || roomCtl.current() === 0) {
        husPaletteId = id;
        applyExterior();
      }
  ...
  // roomCtl finns nu → rita palettväljaren för aktivt rum (rum 0 vid start).
  renderPalett();
